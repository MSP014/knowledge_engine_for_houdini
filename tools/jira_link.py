...
def main():
    parser = argparse.ArgumentParser(description="Simple Jira CLI")
    subparsers = parser.add_subparsers(dest="command", required=True)

    # Get Issue
    p_get = subparsers.add_parser("get", help="Get issue details")
    p_get.add_argument("key", help="Issue Key (e.g. SPL-123)")

    # List Issues
    p_list = subparsers.add_parser("list", help="List open issues for project")
    # DEFAULT CHANGED TO KEH
    p_list.add_argument(
        "project", help="Project Key (e.g. KEH)", nargs="?", default="KEH"
    )
    ...
    # DEFAULT CHANGED TO KEH
    p_create.add_argument("project", help="Project Key", nargs="?", default="KEH")
